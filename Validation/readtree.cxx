...
Bool_t isCNN(Double_t eta, Double_t p, Double_t cov);

void readtree(TString Tree_name, TString path){

TFile* input  = new TFile(path+"/output.root","READ");
TFile* output = new TFile(path+"/Histo_"+Tree_name+".root", "RECREATE");
...
tree->SetBranchAddress("EdepWP", &EdepWP);
tree->SetBranchAddress("EindWP", &EindWP);
tree->SetBranchAddress("LooseID", &Loose);
tree->SetBranchAddress("isCNN", &isCNNTight);
tree->SetBranchAddress("isCNNLoose", &isCNNLoose);
